...
void SbwAnalyzerResults::GenerateBubbleText(U64 frame_index, Channel& channel, DisplayBase display_base)
{
	...
	if (channel == m_pSettings->m_TCKChannel)
	{
		AddResultString(JtagStateStr[frame.mFlags & 0x0f]);
	}
	else if (static_cast<JtagState>(frame.mFlags & 0xf) == JtagState::kJtagShiftDR)
	{
		if (channel == m_pSettings->m_TDIOChannel)
		{
	...
	else if (static_cast<JtagState>(frame.mFlags & 0xf) == JtagState::kJtagShiftIR)
	{
		int idx;
...
void SbwAnalyzerResults::GenerateExportFile(const char* file, DisplayBase display_base, U32 /*export_type_user_id*/)
{
	...
	U64 trigger_sample = m_pAnalyzer->GetTriggerSample();
	U32 sample_rate = m_pAnalyzer->GetSampleRate();
	...
	for (U32 i = 0; i < num_frames; i++)
	{
		...
		if (static_cast<JtagState>(frame.mFlags & 0xf) != JtagState::kJtagShiftDR && static_cast<JtagState>(frame.mFlags & 0xf) != JtagState::kJtagShiftIR)
		{
			continue;
		}
